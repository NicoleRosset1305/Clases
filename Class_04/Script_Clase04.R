...
text(x =G$`Casos confirmados.Femenino`,y=G$`Casos confirmados.Masculino`, G$`Centro de salud`,cex=0.5) ##poner texto en el grafico

#ggplot2
library(ggplot2)

ggplot() ##pagina en blanco para añadir cosas

ggplot(data=E, mapping = aes(x = AvAge,y=`Casos confirmados`)) + geom_point() ##funcion que junta todas las variables que quiero mapear
